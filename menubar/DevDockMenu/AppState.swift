...
class AppState {
    ...
    private func poll() async {
        let healthy = await DevDockAPIClient.shared.checkHealth()

        if healthy {
            isOnline = true
            backoffInterval = normalInterval

            let newProcesses = await DevDockAPIClient.shared.fetchRunningProcesses()
            ...
            profiles = await DevDockAPIClient.shared.fetchProfiles()
        } else {
            if isOnline {
                previousProcessIds = []
                processes = []
    ...
    func stopProcess(_ projectId: String) {
        Task {
            _ = await DevDockAPIClient.shared.stopProcess(projectId: projectId)
            // Next poll will update the list
        }
    }

    func startProfile(_ id: String) {
        Task {
            if let result = await DevDockAPIClient.shared.startProfile(id: id) {
    ...
    func stopProfile(_ id: String) {
        Task {
            _ = await DevDockAPIClient.shared.stopProfile(id: id)
        }
    }

    func openDashboard() {
        if let url = URL(string: "http://localhost:3070") {
